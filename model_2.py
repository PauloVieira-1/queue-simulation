...
# Rates (per minute)
lam = 1/30  # Interarrival rate of student groups (not used in the second scenario)
service_duration =  unif(15, 30)  # Service time is still uniformly distributed between 15 and 30 minutes
# break_duration is no longer used because the coffee break is fixed at 20 minutes in the second scenario
...
def arrive(t, a, w):
    ...
    new_w = w + [token]
    return [
        # For the second scenario, the interarrival time is exactly 30 minutes.
        SimToken(a + 1, delay=30),
...
def should_take_break(free_instructor, current_time):
    if free_instructor == "Instructor" and random() <= 0.7:  
        # For the second scenario, the instructor's coffee break is fixed at 20 minutes.
        print(f"Instructor taking break at {current_time:.2f} for 20.00 mins")
        return [SimToken(None, delay=0),
                SimToken("Instructor", delay=20)]
    return [SimToken(free_instructor, delay=0),
            SimToken([], delay=0)]
